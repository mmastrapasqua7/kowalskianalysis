...
\subsubsection{Pseudocodice dello scraper di Waze}
...
GetRoutes(startLat, startLon, endLat, endLon string) Result {
	...
	// Senza di essi i server non altre interazioni
	HTTPHeaders := HeadersNeededForHTTP(from, to)
	cookie      := MagicCookie(HTTPHeaders)
	fromData    := LocationInfo(from, HTTPHeaders, cookie)
	toData      := LocationInfo(to, HTTPHeaders, cookie)
	
	token  := MagicToken(fromData, toData, HTTPHeaders, cookie)
	routes := SuggestedRoutes(fromData, toData, token, cookie)
...
\subsubsection{Pseudocodice dello scraper di Moovit}
